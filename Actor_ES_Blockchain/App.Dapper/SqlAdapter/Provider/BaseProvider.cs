...
using Dapper;
using System.Text;
using App.Dapper.Core;
using App.Dapper.SqlAdapter;

namespace App.Dapper.SqlAdapter.Provider
{
    public abstract class BaseProvider : IProvider
    {
        ...
        protected Tuple<string,string> GetInsertSqlParts(TableInfo tableInfo)
        {
            ...
            for (int i = 0; i < tableInfo.Columns.Count; i++)
            {
                ...
                if (column.Name.Equals(tableInfo.PrimaryColumnName))
                {
        ...
        public virtual object Insert<T>(IDbConnection connection, TableInfo tableInfo, T data,IDbTransaction transaction = null)
        {
            return null;
        }
        public virtual async Task<object> InsertAsync<T>(IDbConnection connection, TableInfo tableInfo, T data, IDbTransaction transaction = null)
        {
